#include "../Headers/particleNode.hpp"
...
void ParticleNode::updateCurrent(sf::Time dt, CommandQueue&)
{
	...
	for (auto itr = mParticles.begin(); itr < mParticles.end(); ++itr){
		itr->lifetime -= dt;
	}
...
void ParticleNode::computeVertices() const
{
	...
	mVertexArray.clear();
	for (auto itr = mParticles.begin(); itr < mParticles.end(); ++itr){
		sf::Vector2f pos = itr->position;
		sf::Color color = itr->color;

		float ratio = itr->lifetime.asSeconds() / Table[mType].lifetime.asSeconds();
